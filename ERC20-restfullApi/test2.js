...
exports.balanceOf = async(req, res) =>{
        ...
        const provider =  new HDWalletProvider(process.env.MNEMONIC, `https://ropsten.infura.io/v3/${process.env.INFURA_API_KEY}`)
           ...
           res.send(await Contract.methods.balanceOf(req.body.address).call());
...
exports.transfer = async(req, res) =>{
    ...
    const provider =  new HDWalletProvider(process.env.MNEMONIC, `https://ropsten.infura.io/v3/${process.env.INFURA_API_KEY}`)
       ...
       res.json(await Contract.methods.transfer(req.body.recipientAddress, req.body.amount).send({from: acc[0]}));
...
exports.allowance = async(req, res) =>{
    ...
    const provider =  new HDWalletProvider(process.env.MNEMONIC, `https://ropsten.infura.io/v3/${process.env.INFURA_API_KEY}`)
       ...
       res.send(await Contract.methods.allowance(req.body.owner, req.body.spender).call());
...
exports.transferFrom = async(req, res) =>{
    ...
    const provider =  new HDWalletProvider(process.env.MNEMONIC, `https://ropsten.infura.io/v3/${process.env.INFURA_API_KEY}`)
       ...
       res.send(await Contract.methods.transferFrom(req.body.sender, req.body.recipient, req.body.amount).send({from: acc[0]}));
